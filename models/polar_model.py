...
class PolarModel():
    ...
    '''
    connection:DatabaseConnection = DatabaseConnection(database='default', key='default')
    _dbi = PolarsInterface
    dbi:DatabaseProtocol = _dbi(connection)

    @classmethod    
    def connect(cls, connection:DatabaseConnection=None) -> bool:
    ...
    def delete(self,) -> bool:
        return self.dbi.delete(DatabaseConnection(value=self.as_dict))

    @classmethod
    def get(cls, db_connection:DatabaseConnection,
                limit:int=2,
                indexed:bool=False,
                page_number:int=0,
    ...
    def all(cls,
        ...
        return cls.get(DatabaseConnection(database,key,None,None),limit,indexed,page_number)
